...
func _update_harvester_gathering(harvester: Node2D, dt_ms: float) -> void:
	...
	var cargo := int(harvester.get_meta("cargo", 0))
	var can_take := mini(HARVESTER_CAPACITY - cargo, HARVESTER_CAPACITY)
	var taken := 0
	if can_take > 0 and deposit.has_method("harvest"):
		taken = int(deposit.call("harvest", can_take))
	cargo += taken
	harvester.set_meta("cargo", cargo)
	harvester.set_meta("timer_ms", timer)
	_update_civil_progress_line(harvester, float(cargo) / float(HARVESTER_CAPACITY))
	if cargo >= HARVESTER_CAPACITY or not is_instance_valid(deposit):
		_dispatch_harvester_to_hq(harvester)
...
func _update_harvester_unloading(harvester: Node2D, dt_ms: float) -> void:
	...
	if cargo <= 0:
		...
		return
	if _economy.raw >= _economy.raw_cap:
		harvester.set_meta("blocked_reason", "raw-storage-full")
		return
	var timer := float(harvester.get_meta("timer_ms", 0.0)) + dt_ms
	...
	var accepted := _economy.add_raw(cargo)
	cargo -= accepted
	harvester.set_meta("cargo", cargo)
	harvester.set_meta("timer_ms", timer)
	_update_civil_progress_line(harvester, float(cargo) / float(HARVESTER_CAPACITY))
...
func _cancel_selected_factory_item() -> void:
	...
	_feedback_text = "Queue item cancelled"
...
func _handle_unit_hotkey(keycode: Key) -> void:
	match keycode:
		KEY_Q:
			_enter_build_mode("separator")
		KEY_W:
			_enter_build_mode("raw_storage")
		KEY_E:
			_enter_build_mode("matter_storage")
		KEY_R:
			_enter_build_mode("elements_storage")
		KEY_A:
			_enter_build_mode("power_plant")
		KEY_F:
			_enter_build_mode("units_factory")
		KEY_Z:
...
func _build_card_text() -> String:
	return "Build: Q separator | W raw | E matter | R elements | A power | F factory"
...
func _update_rts_hud_text() -> void:
	...
	var build_mode := "none" if _build_mode_id.is_empty() else _build_mode_id

	_hud_label.text = "%s\nFaction: %s | Map: %d x %d | Chunks: %d | Seed: %d\nSelected: %s\nMode: %s | %s\nFeedback: %s\nLMB select/place | RMB move/cancel | wheel zoom | middle drag | Esc" % [
...
func _update_hud_text() -> void:
	...
	var selected_text := "нет"
	if is_instance_valid(_selected_unit) and _selected_unit.has_method("get_status_text"):
		selected_text = str(_selected_unit.call("get_status_text"))

	_hud_label.text = "Фракция: %s | Карта: %d x %d | Чанки: %d | Seed: %d | Юнит: %s | ЛКМ выбрать, ПКМ двигаться/атаковать, колесо двигать камеру, H/T апгрейд, U/I добавить, 1-4 модули, Esc" % [
		player_faction_name,
		map_size,
		map_size,
		_chunk_count,
